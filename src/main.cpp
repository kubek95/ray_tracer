...
#endif
{
    ...
    Projectile proj{Point{0.f, 0.f, 0.f}, Vec3{7.f, 43.f, 0.f}};
    Canvas canvas{300, 300, Color{0.1f, 0.1f, 0.1f}};
    Color projColor{1.f, 0.f, 0.f};
    do {
        printProjectilePosition(proj);
        try {
        canvas.setPixel(static_cast<std::size_t>(proj.position.x()),
                        canvas.height()-static_cast<std::size_t>(proj.position.y()),
                        projColor);
